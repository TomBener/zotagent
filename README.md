...
- PDF extraction runs through [OpenDataLoader PDF](https://github.com/opendataloader-project/opendataloader-pdf), retries failed batches one PDF at a time, and falls back to text-only / `pdftotext` paths for known structural, empty-output, timeout, or low-yield failures. A structured extraction that returns under 50 characters per page on a document of at least four pages counts as low-yield and is re-run through `pdftotext` rather than published — that is the shape a silent extractor failure takes, since it exits cleanly and would otherwise be recorded as `ready`. OpenDataLoader's `tiny` content-safety rule is disabled: it sizes glyphs from `/FontBBox`, and subsetted fonts declaring `[0 0 0 0]` (routine in CNKI exports and other Founder-typeset Chinese PDFs) would otherwise cost the entire body text. EPUB and HTML are extracted in-process; TXT is indexed directly.
